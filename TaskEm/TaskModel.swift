...
struct TaskData: Identifiable {
    ...
    init(id: UUID = UUID(), name: String, icon: Image, iconColor: Color, dueDate: Date? = nil, isComplete: Bool = false){
        self.id = id
        self.name = name
        self.icon = icon
...
extension TaskData {
    static var sampleData: [TaskData] {
        [
            TaskData(name: "Take Out Trash But this one is significantly longer than usual for your title but now we just got even longer like how long can a title genuinly be ?? jeuss", icon: Image(systemName: "trash"), iconColor: .red),
